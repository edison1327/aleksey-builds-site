...
import { Home, Building2, FolderOpen, Truck, Car, Mail, Users, Settings, LayoutDashboard, Info, Briefcase, Heart, Image, Menu, ChevronLeft, ChevronRight, Quote, Navigation, BarChart3, Share2, FileText, TrendingUp, UserCog, MessageSquareQuote, Newspaper, History, CalendarRange, Activity, Database, Bug, FolderLock, Gift, Kanban, MapPin, Webhook, Bell, AlarmClock, Trash2, Wrench, ClipboardCheck, ShoppingCart, WifiOff, FileSignature, Handshake, Zap, AlertTriangle, Brain, KeyRound, ClipboardList, FileSpreadsheet, Layers, Award, Timer, ShieldCheck } from "lucide-react";
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Area, AreaChart, PieChart, Pie, Cell, ResponsiveContainer, Tooltip as RechartsTooltip } from "recharts";
...
const Admin = () => {
    ...
    if (isLoading) {
        return (<div className="min-h-screen bg-gradient-to-br from-background via-muted to-background flex items-center justify-center">
    ...
    const menuItems = allMenuItems.filter((m) => isAdmin || (isManager && m.managerOk) || !m.adminOnly);
    const categories = [
    ...
    const renderSidebar = (isMobile = false) => (<div className="flex flex-col h-full">
      {/* Sidebar Header */}
      <div className={cn("p-4 border-b border-border/50", sidebarCollapsed && !isMobile && "px-2")}>
        {(!sidebarCollapsed || isMobile) ? (<div className="flex items-start gap-2">

            {logoUrl ? (<img src={logoUrl} alt={siteSettings?.company_name || "Logo"} className="h-8 object-contain logo-transition"/>) : (<div className="bg-primary rounded-lg p-2">
            ...
            {!isMobile && (<button onClick={() => setSidebarCollapsed(true)} className="shrink-0 rounded-md p-1.5 text-muted-foreground hover:bg-muted hover:text-foreground transition-colors" title="Ocultar menú" aria-label="Ocultar menú">
                <ChevronLeft className="h-4 w-4"/>
              </button>)}
          </div>) : (<div className="flex flex-col items-center gap-2">
            <button onClick={() => setSidebarCollapsed(false)} className="rounded-lg bg-primary p-2 hover:opacity-90 transition-opacity" title="Mostrar menú" aria-label="Mostrar menú">
      ...
      <ScrollArea className="flex-1 py-4">
        <nav className="space-y-6 px-2">
          {categories.map((category) => (<div key={category.id}>
              {(!sidebarCollapsed || isMobile) && (<p className="px-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">
                  {category.label}
                </p>)}
              <div className="space-y-1">
                ...
                return (<button key={item.id} onClick={() => {
                    ...
                    }} className={cn("w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 relative", activeTab === item.id
                        ? "bg-primary text-primary-foreground shadow-lg shadow-primary/25"
                        : "text-muted-foreground hover:bg-muted hover:text-foreground", sidebarCollapsed && !isMobile && "justify-center px-2")} title={sidebarCollapsed && !isMobile ? `${item.label}${showBadge ? ` (${badgeCount})` : ''}` : undefined}>
                        <div className="relative">
                          <item.icon className="h-5 w-5 shrink-0"/>
                          {showBadge && (<span className="absolute -top-2 -right-2 flex h-5 w-5 items-center justify-center">
                        ...
                        {(!sidebarCollapsed || isMobile) && (<span className="flex items-center gap-2 flex-1 justify-between">
                            <span>{item.label}</span>
                            {showBadge && (<span className={cn("text-[10px] font-bold px-1.5 py-0.5 rounded-full", activeTab === item.id
      ...
      {/* Sidebar Footer */}
      <div className={cn("p-4 border-t border-border/50 space-y-3", sidebarCollapsed && !isMobile && "px-2")}>
        {/* User Menu */}
        <UserMenu collapsed={sidebarCollapsed && !isMobile}/>
        
        {/* Notification Center in sidebar */}
        <div className={cn("flex items-center gap-2", sidebarCollapsed && !isMobile ? "justify-center" : "justify-start")}>
          <NotificationsBell collapsed={sidebarCollapsed && !isMobile} onNavigate={(link) => {
            const hash = link.split("#")[1];
            if (hash)
                setActiveTab(hash);
        }}/>
          {(!sidebarCollapsed || isMobile) && (<span className="text-xs text-muted-foreground">Notificaciones</span>)}
        </div>
        <Link to="/" className="block">
          <Button variant="outline" size="sm" className={cn("w-full gap-2 justify-start", sidebarCollapsed && !isMobile && "justify-center px-2")}>
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-muted/30 to-background flex">
      ...
      <a href="#admin-main" className="sr-only focus:not-sr-only focus:fixed focus:top-2 focus:left-2 focus:z-[100] focus:px-4 focus:py-2 focus:rounded-md focus:bg-primary focus:text-primary-foreground focus:shadow-lg focus:outline-none focus:ring-2 focus:ring-ring">
        Saltar al contenido principal
      </a>
      <CommandPalette open={paletteOpen} onOpenChange={setPaletteOpen} items={menuItems.map((m) => ({
            id: m.id,
            label: m.label,
      ...
      {/* Desktop Sidebar */}
      <aside className={cn("hidden lg:flex flex-col bg-card/95 backdrop-blur-md border-r border-border/50 transition-all duration-300 sticky top-0 h-screen", sidebarCollapsed ? "w-[72px]" : "w-64")}>
        {renderSidebar(false)}
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="lg:hidden bg-card/80 backdrop-blur-md border-b border-border/50 sticky top-0 z-40">
          <div className="flex items-center justify-between px-4 py-3">
            ...
            <div className="flex items-center gap-2 min-w-0">
              {logoUrl ? (<img src={logoUrl} alt={siteSettings?.company_name || "Logo"} className="h-7 object-contain logo-transition"/>) : (<span className="font-heading font-bold text-sm truncate">
            ...
            <div className="flex items-center gap-2">
              ...
              <OfflineIndicator />
              <NotificationsBell onNavigate={(link) => { const h = link.split("#")[1]; if (h)
        setActiveTab(h); }}/>
              <Link to="/">
                <Button variant="ghost" size="icon" className="shrink-0">
        ...
        <main id="admin-main" tabIndex={-1} className="flex-1 p-4 md:p-6 lg:p-8 overflow-auto focus:outline-none">
          <div className="max-w-7xl mx-auto animate-fade-in">
            <ErrorBoundary key={activeTab} fallback={<div className="rounded-lg border border-destructive/30 bg-destructive/5 p-6 text-center">
...
const DashboardOverview = ({ onNavigateToMessages }) => {
    ...
    const [quotesByTypeData, setQuotesByTypeData] = useState([]);
    ...
    useEffect(() => {
        const fetchStats = async () => {
            try {
                ...
                const [servicesRes, projectsRes, pendingMessagesRes, respondedMessagesRes, totalMessagesRes, pendingAppsRes, reviewedAppsRes, totalAppsRes, machineryRes, vehiclesRes, testimonialsRes, recentMessagesRes, recentAppsRes,] = await Promise.all([
                ...
                setMachineryData(Object.entries(machineryByCategory)
                ...
                setVehiclesData(Object.entries(vehiclesByCategory)
                    .map(([category, count]) => ({ category, count }))
                    .sort((a, b) => b.count - a.count));
                ...
                setMessagesData(Object.entries(messagesByMonth).map(([month, count]) => ({ month, count })));
                setQuotesData(Object.entries(quotesByMonth).map(([month, count]) => ({ month, count })));
                const typeColors = {
                    "Maquinaria": "hsl(25, 95%, 53%)",
                    "Vehículos": "hsl(187, 85%, 43%)",
                    "Servicios": "hsl(250, 80%, 60%)",
                };
                setQuotesByTypeData(Object.entries(quotesByType)
    ...
    return (<div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-primary/10 via-primary/5 to-transparent rounded-2xl p-4 md:p-6 border border-primary/20">
        <div className="flex items-center justify-between">
          ...
          <NotificationsBell onNavigate={(link) => { const h = link.split("#")[1]; if (h && onNavigateToMessages && h === "messages")
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        {statCards.map((stat, index) => (<Card key={index} className="border-none shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden group">
            ...
            <CardHeader className="pb-2 relative p-3 md:p-6">
              ...
              <CardTitle className="text-2xl md:text-4xl font-bold">
                {isLoading ? (<span className="inline-block w-8 md:w-12 h-8 md:h-10 bg-muted animate-pulse rounded"/>) : (stat.value)}
              </CardTitle>
            </CardHeader>
        ...
        <Card className="border-none shadow-lg overflow-hidden">
          ...
          <CardContent className="p-4 md:p-6">
            ...
            <div className="space-y-2">
              ...
              {recentMessages.length > 0 ? (recentMessages.slice(0, 4).map((msg) => (<div key={msg.id} className="flex items-center justify-between p-2 bg-muted/30 rounded-lg hover:bg-muted/50 transition-colors">
                  ...
                  </div>))) : (<p className="text-sm text-muted-foreground text-center py-4">No hay mensajes recientes</p>)}
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-none shadow-lg overflow-hidden">
          ...
          <CardContent className="p-4 md:p-6">
            ...
            <div className="space-y-2">
              ...
              {recentApplications.length > 0 ? (recentApplications.slice(0, 4).map((app) => (<div key={app.id} className="flex items-center justify-between p-2 bg-muted/30 rounded-lg hover:bg-muted/50 transition-colors">
                    <div className="min-w-0 flex-1">
                  ...
                  </div>))) : (<p className="text-sm text-muted-foreground text-center py-4">No hay postulaciones recientes</p>)}
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
        <Card className="border-none shadow-lg overflow-hidden">
          ...
          <CardContent className="pt-6 p-4 md:p-6">
            {isLoading ? (<div className="h-48 md:h-64 bg-muted animate-pulse rounded-lg"/>) : (<ChartContainer config={{
                ...
                <BarChart data={messagesData} barCategoryGap="20%">
                  ...
                  <XAxis dataKey="month" tick={{ fontSize: 11, fill: 'hsl(var(--muted-foreground))' }} tickLine={false} axisLine={false} dy={8}/>
                  <YAxis tick={{ fontSize: 11, fill: 'hsl(var(--muted-foreground))' }} tickLine={false} axisLine={false} allowDecimals={false} dx={-8}/>
                  <ChartTooltip content={<ChartTooltipContent />} cursor={{ fill: 'hsl(var(--muted))', opacity: 0.3 }}/>
                  <Bar dataKey="count" fill="url(#messagesGradient)" radius={[8, 8, 0, 0]} maxBarSize={50}/>
                </BarChart>
              </ChartContainer>)}
          </CardContent>
        </Card>
        ...
        <Card className="border-none shadow-lg overflow-hidden">
          ...
          <CardContent className="pt-6 p-4 md:p-6">
            {isLoading ? (<div className="h-48 md:h-64 bg-muted animate-pulse rounded-lg"/>) : projectsData.length > 0 ? (<ChartContainer config={{
                ...
                <BarChart data={projectsData} barCategoryGap="20%">
                  ...
                  <CartesianGrid strokeDasharray="3 3" vertical={false} className="stroke-muted/50"/>
                  <XAxis dataKey="year" tick={{ fontSize: 11, fill: 'hsl(var(--muted-foreground))' }} tickLine={false} axisLine={false} dy={8}/>
                  <YAxis tick={{ fontSize: 11, fill: 'hsl(var(--muted-foreground))' }} tickLine={false} axisLine={false} allowDecimals={false} dx={-8}/>
                  <ChartTooltip content={<ChartTooltipContent />} cursor={{ fill: 'hsl(var(--muted))', opacity: 0.3 }}/>
                  <Bar dataKey="count" fill="url(#projectsGradient)" radius={[8, 8, 0, 0]} maxBarSize={50}/>
                </BarChart>
              </ChartContainer>) : (<div className="h-48 md:h-64 flex items-center justify-center text-muted-foreground bg-gradient-to-br from-muted/30 to-muted/10 rounded-xl border border-dashed border-border">
        ...
        <CardContent className="pt-6 p-4 md:p-6">
          {isLoading ? (<div className="h-48 md:h-64 bg-muted animate-pulse rounded-lg"/>) : (<ChartContainer config={{
              ...
              <AreaChart data={quotesData}>
                ...
                <Area type="monotone" dataKey="count" stroke="hsl(350, 89%, 60%)" strokeWidth={3} fill="url(#quotesGradient)" dot={{ fill: 'hsl(350, 89%, 60%)', strokeWidth: 0, r: 4 }} activeDot={{ r: 6, fill: 'hsl(350, 89%, 60%)', stroke: 'hsl(var(--background))', strokeWidth: 2 }}/>
              </AreaChart>
            </ChartContainer>)}
        </CardContent>
      </Card>
      ...
      <Card className="border-none shadow-lg overflow-hidden">
        ...
        <CardContent className="pt-6 p-4 md:p-6">
          {isLoading ? (<div className="h-48 md:h-64 bg-muted animate-pulse rounded-lg"/>) : quotesByTypeData.length > 0 ? (<div className="flex flex-col md:flex-row items-center gap-6">
              <div className="w-full md:w-1/2 h-48 md:h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={quotesByTypeData} cx="50%" cy="50%" innerRadius={60} outerRadius={90} paddingAngle={4} dataKey="count" strokeWidth={0}>
                      {quotesByTypeData.map((entry, index) => (<Cell key={`cell-${index}`} fill={entry.fill}/>))}
                    </Pie>
                    <Tooltip formatter={(value, name, props) => [`${value} solicitudes`, props.payload.type]} contentStyle={{
              ...
              <div className="w-full md:w-1/2 space-y-3">
                {quotesByTypeData.map((item, index) => (<div key={index} className="flex items-center justify-between p-3 rounded-lg bg-muted/30 hover:bg-muted/50 transition-colors">
            ...
            </div>) : (<div className="h-48 md:h-64 flex items-center justify-center text-muted-foreground bg-gradient-to-br from-muted/30 to-muted/10 rounded-xl border border-dashed border-border">
              <div className="text-center">
                <div className="p-3 rounded-full bg-muted/50 w-fit mx-auto mb-3">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
        <Card className="border-none shadow-lg overflow-hidden">
          ...
          <CardContent className="pt-6 p-4 md:p-6">
            {isLoading ? (<div className="h-48 md:h-64 bg-muted animate-pulse rounded-lg"/>) : machineryData.length > 0 ? (<ChartContainer config={{
                ...
                <BarChart data={machineryData} barCategoryGap="20%" layout="vertical">
                  ...
                  <YAxis type="category" dataKey="category" tick={{ fontSize: 10, fill: 'hsl(var(--muted-foreground))' }} tickLine={false} axisLine={false} width={90}/>
                  <ChartTooltip content={<ChartTooltipContent />} cursor={{ fill: 'hsl(var(--muted))', opacity: 0.3 }}/>
                  <Bar dataKey="count" fill="url(#machineryGradient)" radius={[0, 8, 8, 0]} maxBarSize={30}/>
                </BarChart>
              </ChartContainer>) : (<div className="h-48 md:h-64 flex items-center justify-center text-muted-foreground bg-gradient-to-br from-muted/30 to-muted/10 rounded-xl border border-dashed border-border">
                <div className="text-center">
                  <div className="p-3 rounded-full bg-muted/50 w-fit mx-auto mb-3">
        ...
        <Card className="border-none shadow-lg overflow-hidden">
          ...
          <CardContent className="pt-6 p-4 md:p-6">
            {isLoading ? (<div className="h-48 md:h-64 bg-muted animate-pulse rounded-lg"/>) : vehiclesData.length > 0 ? (<ChartContainer config={{
                ...
                <BarChart data={vehiclesData} barCategoryGap="20%" layout="vertical">
                  ...
                  <YAxis type="category" dataKey="category" tick={{ fontSize: 10, fill: 'hsl(var(--muted-foreground))' }} tickLine={false} axisLine={false} width={90}/>
                  <ChartTooltip content={<ChartTooltipContent />} cursor={{ fill: 'hsl(var(--muted))', opacity: 0.3 }}/>
                  <Bar dataKey="count" fill="url(#vehiclesGradient)" radius={[0, 8, 8, 0]} maxBarSize={30}/>
                </BarChart>
              </ChartContainer>) : (<div className="h-48 md:h-64 flex items-center justify-center text-muted-foreground bg-gradient-to-br from-muted/30 to-muted/10 rounded-xl border border-dashed border-border">
                <div className="text-center">
                  <div className="p-3 rounded-full bg-muted/50 w-fit mx-auto mb-3">
